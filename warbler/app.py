...
def add_like(msg_id):
    ...
    if liked_msg in user_likes:
        g.user.likes = [like for like in user_likes if like != liked_msg] #recreates list without the unliked message
    else:
        g.user.likes.append(liked_msg) #if not in the list, add it
...
def profile():
    ...
    if edit_form.validate_on_submit():
        if User.authenticate(edit_form.username.data, edit_form.password.data):
